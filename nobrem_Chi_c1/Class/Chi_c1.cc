...
////////////////////////////// # of tracks == 2 /////////////////////////////////
         ...
         ++trackItr )
   {
      ...
      if(trackItr.valid()) {
        ...
        if(electronFit.valid() && electronFit->pmag() > 1.4) {

          CHARGE[i]=electronFit->charge();
          PEee[i]=electronFit->energy();
          PXee[i]=electronFit->px();
          PYee[i]=electronFit->py();
          PZee[i]=electronFit->pz();
          PMAGee[i]=electronFit->pmag();
          tr_ee[i]=electronFit;
        }
        else ISee = false;

        if(muonFit.valid() && muonFit->pmag() > 1.4) {
...
//// ee with no mass constraint ///////
   ...
   psip_ee.newTrack(*tr_ee[0]);
   psip_ee.newTrack(*tr_ee[1]);
   psip_ee.doTheFit();
...
//// ee with mass constraint /////
   ...
   jpsi_ee.newTrack(*tr_ee[0]);
   jpsi_ee.newTrack(*tr_ee[1]);
   jpsi_ee.doTheFit();
