...
// user include files
#include "MuMuJpsiProc/MuMuJpsiProc.h"
...
//
static const char* const kFacilityString = "Processor.MuMuJpsiProc" ;
...
//
MuMuJpsiProc::MuMuJpsiProc( void )               // anal1
   : Processor( "MuMuJpsiProc" )
{
   ...
   bind( &MuMuJpsiProc::event,    Stream::kEvent );
   bind( &MuMuJpsiProc::beginRun, Stream::kBeginRun );
   //bind( &MuMuJpsiProc::endRun,   Stream::kEndRun );
...
MuMuJpsiProc::~MuMuJpsiProc()                    // anal5
{
...
// --------------------------- init method -------------------------
void
MuMuJpsiProc::init( void )          // anal1 "Interactive"
{
...
// -------------------- terminate method ----------------------------
void
MuMuJpsiProc::terminate( void )     // anal5 "Interactive"
{
...
void
MuMuJpsiProc::hist_book( HIHistoManager& iHistoManager )
{
...
MuMuJpsiProc::event( Frame& iFrame )          // anal3 equiv.
{
...
*/
         ...
         ++trackItr )
   {
      ...
      if(pionFit == tagpi1 || pionFit == tagpi2) {
         pion_tracks.push_back(pionFit);
      }
      else if((pionFit->energy() > tagpi1->energy()) && (pionFit->energy() > tagpi2->energy()) ) {
...
*/
   ...
   for(int i=0; i<2; ++i) 
     psip.newTrack(*pion_tracks[i]);
   for(int i=0; i<2; ++i) 
     psip.newTrack(*muon_tracks[i]);
...
ActionBase::ActionResult
MuMuJpsiProc::beginRun( Frame& iFrame )       // anal2 equiv.
{
...
/*
ActionBase::ActionResult
MuMuJpsiProc::endRun( Frame& iFrame )         // anal4 equiv.
{
...
int MuMuJpsiProc::CompareKins( FAItem<KTKinematicData>  &k1, 
...
/*
// return 1 if they are they same kinematical object
int MuMuJpsiProc::CompareKins( FAItem<TDKinematicFit>  &k1, 
...
double MuMuJpsiProc::calcDOCA( double iD,
		       	     double iPhi )
{
