// -*- C++ -*-
//
// Package:     FourTrPi0JpsiProc
// Module:      FourTrPi0JpsiProc
// 
...
// user include files
#include "FourTrPi0JpsiProc/FourTrPi0JpsiProc.h"
...
//
static const char* const kFacilityString = "Processor.FourTrPi0JpsiProc" ;
...
//
FourTrPi0JpsiProc::FourTrPi0JpsiProc( void )               // anal1
   : Processor( "FourTrPi0JpsiProc" )
{
   ...
   bind( &FourTrPi0JpsiProc::event,    Stream::kEvent );
   bind( &FourTrPi0JpsiProc::beginRun, Stream::kBeginRun );
   //bind( &FourTrPi0JpsiProc::endRun,   Stream::kEndRun );
...
FourTrPi0JpsiProc::~FourTrPi0JpsiProc()                    // anal5
{
...
// --------------------------- init method -------------------------
void
FourTrPi0JpsiProc::init( void )          // anal1 "Interactive"
{
...
// -------------------- terminate method ----------------------------
void
FourTrPi0JpsiProc::terminate( void )     // anal5 "Interactive"
{
...
void
FourTrPi0JpsiProc::hist_book( HIHistoManager& iHistoManager )
{
...
FourTrPi0JpsiProc::event( Frame& iFrame )          // anal3 equiv.
{
...
//            && (pi0Itr->loEnShower().identifier() != fastest_shower->identifier()) ) {
            ...
            if( (psip.chisqVtx()>0) && (psip.chisqFit()>0) ) {
		     ...
		     HepLorentzVector v5pi (psip.kdFitVec()[5]->lorentzMomentum());
	             HepLorentzVector v6pi0 (psip.kdFitVec()[6]->lorentzMomentum());
   ...
   // do fit for K+K-
   if( (kaon_tracks.size()==2) && (pion_tracks.size()==4) ) {
...
ActionBase::ActionResult
FourTrPi0JpsiProc::beginRun( Frame& iFrame )       // anal2 equiv.
{
...
/*
ActionBase::ActionResult
FourTrPi0JpsiProc::endRun( Frame& iFrame )         // anal4 equiv.
{
...
int FourTrPi0JpsiProc::CompareKins( FAItem<KTKinematicData>  &k1, 
...
/*
// return 1 if they are they same kinematical object
int FourTrPi0JpsiProc::CompareKins( FAItem<TDKinematicFit>  &k1, 
...
double FourTrPi0JpsiProc::calcDOCA( double iD,
		       	     double iPhi )
{
